...
func AesEncrypt(msg, key []byte, appId string) (string, error) {
	all := make([]byte, 0)
	randBs := make([]byte, 16)
	io.ReadFull(rand.Reader, randBs)

	lenBs := make([]byte, 4)
	binary.BigEndian.PutUint32(lenBs, uint32(len(msg)))

	all = append(all, randBs...)        // 16 rand bytes
	all = append(all, lenBs...)         // 4 length bytes
	all = append(all, []byte(msg)...)   // msg content
...
func AesDecrypt(s string, key []byte) (*proto.RawMsg, error) {
	cipherData, err := base64.StdEncoding.DecodeString(s)
	if err != nil {
		return nil, err
	}
